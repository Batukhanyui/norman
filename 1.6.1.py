...
T = 2 #температура
massa = 1
velocity = int(1.7*sqrt(3*T/massa) + 0.5) #предел начальной скорости
...
def potentionalenergy():
    result = 0.
    for qel in range(n):
        for qol in range(qel+1, n):
            radius_vector = vector(coordinate[qel], coordinate[qol])
            dist = radius_vector[0]**2 + radius_vector[1]**2 + radius_vector[2]**2
            f = (radius**2/dist)**3
            result += f*(f - 1)
    return 4*E*result
...
while not control:
    control = True
    for ul in range(n):
        for el in range(ul+1, n):
            if distancesquare(coordinate[ul], coordinate[el]) < radius**2:
                control = False
                for ol in range(3):
...
for ul in range(time):
    coordinate += velocitys*dt + 0.5*dt*acceleration1
    for el in range(n):
         acceleration2[el] = acceleration(el)
    velocitys += 0.5*(acceleration1 + acceleration2)    
